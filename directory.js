...
} // Line ~14. Check syntax here.

// === Helper Function: UTF-8 to Base64 Encoding/Decoding (Re-added Meticulously) ===
// Encodes a standard JS string (UTF-16) to Base64 via UTF-8 bytes
function utf8ToBase64(str) { // Line ~19. Check syntax here.
    try {
        // Ensure input is treated as a string (handles null/undefined)
        ...
        // Use Array.from and map, then join for safer conversion from byte array
        const binaryString = Array.from(utf8Bytes, byte => String.fromCharCode(byte)).join(''); // Using Array.from+map+join
...
} // Line ~36. Check closing brace.

// Decodes a Base64 string back to a standard JS string (UTF-16) via UTF-8 bytes
function base64ToUtf8(base64) { // Line ~39. Check syntax.
    try {
...
// ======================================================================
async function fetchAndDisplayListings() { // Line ~58. Check syntax.
    // Check for global client
    if (typeof supabaseClient === 'undefined' || !supabaseClient) {
        displayError("Supabase client not initialized (from common.js). Cannot fetch data.");
        return; // Line ~63. Check return and closing brace.
    } // Line ~64. Check closing brace.
    console.log("Directory.js using supabaseClient initialized in common.js"); // Line ~65. This is near the error line. Check this line and the one immediately before/after carefully.
        ...
        sortedCategories.forEach(category => {
             ...
             categorySortedListings.forEach(listing => {
                ...
                try {
                    ...
                    // This function needs to be defined *before* this is called
